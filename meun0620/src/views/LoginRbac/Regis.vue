  ...
  data() {
    return {
      ruleForm: {
        ...
        admPwd: "", //密码
        admEmile: "", //邮箱
        // addDateTimeA: null, //登录日期 //加上时间字段报 400  原因：类型不能转换   解决方案：把类型转换成'后端'的类型一致
        // lastLoginDateTimeA: null, //末次登录时间
        lastLoginIPA: "", //上次登录
        isLos: false,
      },
      rules: {
        admName: [{ required: true, message: "请输入账号", trigger: "blur" }],
        // admEmile: [
        //   { required: true, message: "请输入邮箱", trigger: "blur" },
        // ],
        admPwd: [{ required: true, message: "请输入密码", trigger: "blur" }],
      },
    };
  },
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http
            .post(
              "https://localhost:44343/api/LoginServer/Register",
              this.ruleForm
            )
            .then((res) => {
              let infor = res.data;
